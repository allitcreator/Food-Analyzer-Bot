...
export function generateMonthlyPDF(
  ...
  const avgCal = weeklyStats.filter(w => w.days > 0).reduce((s, w) => s + w.calories, 0) / Math.max(weeklyStats.filter(w => w.days > 0).length, 1);

  const boxes = [
    { label: 'Дней\nзалогировано', value: String(totalActiveDays) },
    { label: 'Ср. калорий\nв день', value: avgCal > 0 ? Math.round(avgCal).toString() : '—' },
    { label: 'Цель\n(ккал)', value: user.caloriesGoal ? String(user.caloriesGoal) : '—' },
    { label: 'Записей\nо весе', value: String(weightLogs.length) },
  ];
  ...
  const drawBarChart = (
    ...
    if (y + chartH + 30 > 780) { doc.addPage(); y = 50; }
    const n = data.length;
    const barW = Math.min(36, (W - 20) / n - 8);
    ...
    data.forEach((item, i) => {
      ...
      if (item.value > 0) {
        doc.fillColor(DARK).font('Helvetica').fontSize(7)
          .text(String(item.value), bx - 2, by - 10, { width: barW + 4, align: 'center' });
      }
    ...
    // goal line if calories
    if (maxVal > 0 && data.some(d => d.value > 0)) {
      doc.moveTo(50, y).lineTo(50 + W, y).stroke(LIGHT);
    }
  ...
  if (user.caloriesGoal) {
    doc.fillColor(GRAY).font('Helvetica').fontSize(9)
      .text(`Цель: ${user.caloriesGoal} ккал/день`, 50, y - 20, { align: 'right', width: W });
  }

  // ─── КБЖУ по неделям (table) ───────────────────────────────────────────────
  sectionTitle('КБЖУ по неделям (среднее в день)');

  // table header
  const cols = [{ label: 'Неделя', w: 130 }, { label: 'Дней', w: 45 }, { label: 'Ккал', w: 60 }, { label: 'Белки г', w: 60 }, { label: 'Жиры г', w: 60 }, { label: 'Углев г', w: 60 }];
  let tx = 50;
  ...
  weeklyStats.forEach((wk, idx) => {
    if (y + 20 > 780) { doc.addPage(); y = 50; }
    const bg = idx % 2 === 0 ? BG : 'white';
  ...
  // ─── Daily calorie chart (last 7 days) ────────────────────────────────────
  ...
  if (weightLogs.length >= 2) {
    ...
    if (y + chartH + 40 > 780) { doc.addPage(); y = 50; }

    // grid lines
    for (let g = 0; g <= 4; g++) {
      const gy = y + chartH - (g / 4) * chartH;
      const gv = minW + (g / 4) * (maxW - minW);
      doc.moveTo(50, gy).lineTo(50 + chartW, gy).strokeColor(LIGHT).lineWidth(0.5).stroke();
      doc.fillColor(GRAY).font('Helvetica').fontSize(7)
    ...
    const points = sorted.map((l, i) => ({
      x: 50 + (i / Math.max(sorted.length - 1, 1)) * chartW,
    ...
    points.forEach((p, i) => {
      ...
      doc.fillColor(GRAY).font('Helvetica').fontSize(7)
        .text(dl, p.x - 12, y + chartH + 5, { width: 26, align: 'center' });
      doc.fillColor(DARK).fontSize(7)
        .text(sorted[i].weight.toFixed(1), p.x - 12, p.y - 14, { width: 26, align: 'center' });
    });
  ...
  if (topFoods.length > 0) {
    ...
    if (y + topFoods.length * 18 + 10 > 780) { doc.addPage(); y = 50; }
  ...
  const ACT_LABEL: Record<string, string> = { sedentary: 'Сидячий', light: 'Лёгкая активность', moderate: 'Умеренная', active: 'Активный', very_active: 'Очень активный' };
  ...
  const profileLines = [
    ...
    user.caloriesGoal ? `Норма: ${user.caloriesGoal} ккал | Б:${user.proteinGoal}г Ж:${user.fatGoal}г У:${user.carbsGoal}г` : null,
  ].filter(Boolean) as string[];

  profileLines.forEach(line => {
    if (y + 16 > 780) { doc.addPage(); y = 50; }
