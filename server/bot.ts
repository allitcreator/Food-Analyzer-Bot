...
export function setupBot(storage: IStorage, app?: import("express").Express): TelegramBot {
  ...
  bot.onText(/^\/healthsetup(@\w+)?$/, async (msg) => {
    ...
    const text = [
      ...
      ``,
      `1️⃣ Действие «Найти образцы здоровья» → Шаги → Сегодня → Сумма`,
      `2️⃣ Действие «Найти образцы здоровья» → Активная энергия → Сегодня → Сумма`,
      `3️⃣ Действие «Получить содержимое URL»:`,
      `   • URL: ваш URL из /sync`,
      `   • Метод: POST | Тело: JSON`,
      `   • Поля: \`steps\` и \`active_calories\``,
    ].join("\n");
